...
namespace Ninhao.BLL
{
    public class UserManager
    {
        ...
        public static async Task CreateAnonymousUser(UserInformationDTO user, Guid? carid)
        {
            using (var userSvc = new DAL.UserService())
            {
                await userSvc.CreateAsync(new User()
        ...
        public static async Task ChangeDriverCar(Guid userid, Guid id)
        {
            using (var userSvc = new UserService())
            {
                await userSvc.ChangeDriverCar(userid, id);
            }
        }
    }
}